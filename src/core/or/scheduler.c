...
select_scheduler(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(get_options()->SchedulerTypes_, int *, type) {
    ...
    case SCHEDULER_KIST_LITE:
      ...
      goto end;
    case SCHEDULER_PRIV_VANILLA:
      new_scheduler = get_privacy_vanilla_scheduler();
      goto end;
    case SCHEDULER_PRIV_KIST:
  ...
  log_debug(LD_SCHED, "Scheduler type %s has been selected.",
...
set_scheduler(void)
{
  ...
  log_debug(LD_SCHED, "Scheduler changed from %p to %p",
             old_scheduler, the_scheduler);
