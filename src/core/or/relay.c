                           ...
                           cell_direction_t cell_direction)
{
  ...
  log_info(LD_OR, "[Cell] Added to circuit queue");

  /*
   * If "DuummyCellGenProbability" is defined and > 0, there is a
   * differential private chance to add a Packet Padding Cell to the real
   * cell circuit queue.
   *
   * Only relay cells can trigger dummy cells generation, making control
   * and other circuits free from this logic and mechanism.
   */
  if ((cell->command == CELL_RELAY || cell->command == CELL_RELAY_EARLY)) {
    if (dp_generate_bool(false, get_options()->DummyCellEpsilon)) {
      cell_t dummy;
      memset(&dummy, 0, sizeof(cell_t));
      dummy.command = CELL_DUMMY;
      dummy.circ_id = cell->circ_id;
      crypto_fast_rng_getbytes(get_thread_fast_rng(), dummy.payload,
                               sizeof(dummy.payload));
      if (append_cell_to_circuit_queue(circ, chan, &dummy, cell_direction, 0) <
          0) {
        log_warn(LD_OR,
                 "[Packet Padding Cell] Dummy Cell not added sucessfully!");
      } else {
        log_info(
            LD_OR,
            "[Packet Padding Cell] Dummy Cell added to queue sucessfully!");
      }
      // append_dummy_cell_to_circuit_queue(circ, chan, cell, cell_direction,
      // 0);
    }
  }
           ...
           const char *filename, int lineno))
{
  ...
  return append_cell_to_circuit_queue(circ, chan, cell, cell_direction,
                                      on_stream);
}
...
MOCK_IMPL(int, relay_send_command_from_edge_,
          (streamid_t stream_id, circuit_t *orig_circ, uint8_t relay_command,
           ...
           const char *filename, int lineno))
{
  ...
  int ret = circuit_package_relay_cell(
      &cell, circ, cell_direction, cpath_layer, stream_id, filename, lineno);
  if (ret < 0) {
                                   ...
                                   crypt_path_t *layer_hint)
{
  ...
  if (!circ->conflux || !conflux_should_multiplex(rh.command)) {
    ...
    if (conflux_process_cell(circ->conflux, circ, layer_hint, cell)) {
      ...
      if ((ret = connection_edge_process_ordered_relay_cell(
               cell, circ, conn, layer_hint, &rh)) < 0) {
        return ret;
      }
      ...
      while ((c_cell = conflux_dequeue_cell(circ->conflux))) {
        ...
        conn = relay_lookup_conn(circ, &c_cell->cell, CELL_DIRECTION_OUT,
                                 layer_hint);
        if ((ret = connection_edge_process_ordered_relay_cell(
                 &c_cell->cell, circ, conn, layer_hint, &rh)) < 0) {
...
void
cell_queue_append_packed_dummy(circuit_t *circ, cell_queue_t *queue,
                               int exitward, const cell_t *cell,
                               int wide_circ_ids, int use_stats)
{
  if ((cell->command == CELL_RELAY || cell->command == CELL_RELAY_EARLY)) {
    ...
    crypto_fast_rng_getbytes(get_thread_fast_rng(), dummy.payload,
                             sizeof(dummy.payload));
    cell_queue_append_packed_copy(circ, queue, exitward, &dummy, wide_circ_ids,
                                  use_stats);

    log_debug(LD_OR, "DUMMY Dummy cell added to queue for circ %d",
              dummy.circ_id);
  }
}
                            ...
                            int block, streamid_t stream_id)
{
  ...
  for (edge_connection_t *edge = stream_list; edge; edge = edge->next_stream) {
    ...
    if (!conn->read_event || edge->xoff_received || conn->marked_for_close) {
                             ...
                             cell_direction_t direction, streamid_t fromstream)
{
  ...
  if (PREDICT_UNLIKELY(queue->n >= max_queue_size)) {
    ...
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
           ...
           (exitward) ? "Outbound" : "Inbound", queue->n, max_queue_size);
...
int
append_dummy_cell_to_circuit_queue(circuit_t *circ, channel_t *chan,
                                   ...
                                   streamid_t fromstream)
{
  ...
  /* Very important that we copy to the circuit queue because all calls to
   * this function use the stack for the cell memory. */
  cell_queue_append_packed_dummy(circ, queue, exitward, cell,
                                 chan->wide_circ_ids, 1);
