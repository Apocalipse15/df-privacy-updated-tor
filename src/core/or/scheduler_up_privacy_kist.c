 ...
 */
static void
privacy_up_kist_scheduler_set_next_run(unsigned int *sched_interval) 
{
  priv_sched_next_run =
...
set_scheduler_run_interval(void)
{
  ...
  privacy_up_kist_scheduler_set_next_run(&priv_sched_next_run);
...
static void
privacy_up_kist_scheduler_on_new_options(void)
{
...
static void
privacy_up_kist_scheduler_init(void)
{
  ...
  privacy_up_kist_scheduler_on_new_options();
...
static void
privacy_up_kist_scheduler_schedule(void)
{
...
static void
privacy_up_kist_scheduler_run(void)
{
  /* Define variables */
...
static scheduler_t privacy_kist_scheduler = {
  .type = SCHEDULER_UP_PRIV_KIST,
  .free_all = kist_free_all,
  .on_channel_free = kist_on_channel_free_fn,
  .init = privacy_up_kist_scheduler_init,
  .on_new_consensus = kist_scheduler_on_new_consensus,
  .schedule = privacy_up_kist_scheduler_schedule,
  .run = privacy_up_kist_scheduler_run,
  .on_new_options = privacy_up_kist_scheduler_on_new_options,
};

/* Return the KIST scheduler object. If it didn't exists, return a newly
 * allocated one but init() is not called. */
scheduler_t *
get_privacy_up_kist_scheduler(void)
{
