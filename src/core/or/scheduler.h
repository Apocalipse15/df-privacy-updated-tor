...
/* Default Privacy Scheduler Interval */
#define PRIV_SCHED_DEFAULT_JITTER 2
/* Maximum interval that Privacy Scheduler runs (in ms) */
#define PRIV_SCHED_MAX_JITTER 10
/* Minimum interval that Privacy Scheduler runs (in ms) */
#define PRIV_SCHED_MIN_JITTER 1

typedef struct scheduler_jitter_t {
  /** The amount of jitter to add to the scheduler run interval. */
  int32_t target;
  /** The maximum run interval for the scheduler. */
  int32_t max;
  /** The minimum run interval for the scheduler. */
  int32_t min;
  /** Probability of using the jitter. */
  int32_t probability; 
} scheduler_jitter_t;
...
/*********************************
 * Defined in scheduler_privacy_vanilla.c
 *********************************/
scheduler_t *get_privacy_vanilla_scheduler(void);
