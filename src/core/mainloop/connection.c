  ...
  if (connection_speaks_cells(conn) &&
    ...
    CASE_TOR_TLS_ERROR_ANY_NONIO:
      log_debug(LD_NET, "tls error [%s] from %s. Breaking.",
                tor_tls_err_to_string(result), connection_describe(conn));
      return result;
    ...
    if (result < 0) {
      ...
      log_info(LD_NET, "[%d] TLS_RECEIVED: time=%ld.%09ld, from=%s", result,
               ts.tv_sec, ts.tv_nsec, connection_describe_peer(conn));
    }
